...
export const calculateProductsQuantity = (productList:ProductQuantity[])=>{
    ...
    for(let product of productQuantity.keys()){
        switch (product){
            case 1:
                result.push({product:new Apple(),quantity:productQuantity.get(product) })
                break;
            case 2:
                result.push({product:new Banana(),quantity:productQuantity.get(product) })
                break;
            case 3:
                result.push({product:new Kiwi(),quantity:productQuantity.get(product) })

                break;
            case 4:
                result.push({product:new Peach(),quantity:productQuantity.get(product) })
                break;
        }
    }
    return result;
