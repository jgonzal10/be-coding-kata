...
export const calculateTotalPricePerProduct= (productQuantity: number, product: Product ): number =>{
    let isProductQuantiyDiscountable =false;
    const productPriceWithoutDiscount = productQuantity * product.price;

if(product.offerQuantiy! && product.offerPrice!){// Checking if the product has a offer 
